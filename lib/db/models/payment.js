...
paymentSchema.pre('save', async function(next) {
    if (this.isNew) {
        ...
        this.paymentId = `PAY-${Date.now()}-${Math.random().toString(36).substr(2, 6).toUpperCase()}`;
        
        // Generate reference number
        this.referenceNumber = `REF-${Date.now()}-${Math.random().toString(36).substr(2, 8).toUpperCase()}`;
    }
    next();
});
