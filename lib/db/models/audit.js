...
const auditSchema = new mongoose.Schema({
    ...
    // Action type: login, verify, send, reauth success/failure
    // We include reauth_success and reauth_failure so endpoints that write
    // short-lived reauth audit entries do not fail validation. Keep this
    // enum in sync with any places that create audit records.
    action: {
        type: String,
        enum: ['login', 'verify', 'send', 'reauth_success', 'reauth_failure'],
        required: true
    },
